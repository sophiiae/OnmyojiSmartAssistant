from PyQt6.QtWidgets import QGroupBox, QVBoxLayout, QLineEdit, QComboBox, QDoubleSpinBox, QCheckBox
...
class ScriptSection(QGroupBox):
    def __init__(self, config):
        ...
        self.screenshot_interval_spin.setValue(
            opt.get("screenshot_interval", 0.3))
        add_labeled_widget(layout, "截图间隔", self.screenshot_interval_spin)
        ...
        self.combat_interval_spin.setValue(
            opt.get("combat_screenshot_interval", 1.0))
        add_labeled_widget(layout, "战斗截图间隔", self.combat_interval_spin)
